...
- **`Char` renders as a single-quoted char-literal in Debug output.** `@derive(Debug)` used to print a `Char` as its bare code point (`'a'` came out as `97`); a panel resolved the debug-form to the quoted-literal form — `'a'`, `'\n'`, `'\''` — which is injective (a `Char` is now visually distinct from an `Int` or a 1-char `Str`) and applies at every derive position (struct field, list element, option inner, map key), so `Map[Char, Int]` renders as `{'a': 1}`.
- **Non-Debug field types are rejected with a clear E0520.** `Set`, `Result`, and now `Fn`/`Tuple`/`Handler` field types (which have no derivable `Debug` in v1) are rejected at every level of nesting with **E0520**, guiding you to implement `Debug` manually, instead of slipping through to a compiler-bug marker.
- **`import blink.ffi` and `import blink.core` now resolve.** The spec points users at both (§9.1.1 for FFI pointer types, §07 for `ConversionError`), but the imports used to throw `ModuleNotFound`. They now resolve as inert documentation markers — the `ffi`/`core` namespaces remain no-import intrinsics like `io`/`net`/`time`, so existing no-import FFI code is unaffected and the import is purely optional.
- **E0910 for trait-impl argument-arity mismatch.** An `impl` that supplies the wrong number of type arguments for a trait (e.g. `impl Marker[Int, Str] for Foo` on a `trait Marker[T]`, or a wrong-arity `From2[T]`) now errors with **E0910** and an accurate "supplies N but declares M" message, instead of either being silently accepted (methodless traits) or leaking an unbound trait type-parameter name into a misleading "expected 'U'" error. `Iterator`/`IntoIterator` are now covered by the check too.
- **E1401 for non-derivable `@derive(Hash/Eq/Ord)` fields.** A `@derive(Hash/Eq/Ord)` type may only contain fields whose type implements the derived trait (§3.6.6). Collections (`List`/`Map`/`Set`/`Option`/`Result`) have no value hash/eq/ord in v1, `Float` isn't Hashable, and `Bool` isn't Orderable — such a field now errors with **E1401** instead of emitting a degenerate constant-hash sentinel that silently broke hashing.

## Fixes (v0.53.0)

- **Arena promotion of `Map`/`List` with non-`Str`, narrow-int, or `Char` keys/elements no longer corrupts data.** A `Map` or `List` returned out of a `with arena { }` block (or captured by a returned closure) previously promoted with the wrong key-ops table and stride: non-`Str` keys were read as `char*`, and narrow ints (`I8`/`I16`/`I32`, `U8`…) and `Char` were widened to 8 bytes and over-read. Every subsequent lookup missed — `Map[Int, Int]` / `Map[Bool, Int]` segfaulted, `Map[Char, _]` panicked with "unwrap called on None". The promoter now carries the exact key/element width at every seam, so these maps and lists survive arena promotion intact.
- **Tuple / `Option` / `Result` carrier codegen no longer emits invalid C.** A batch of typecheck-accepts-but-`cc`-fails traps around tuple and Option/Result carriers are fixed: matching a tuple whose element is a struct, data enum, or `Option`/`Result` (used to bind the element as `void`); a `Tuple[..]` struct field (emitted a generic `void`/`blink_Tuple`); `Option[Tuple]`/`Result[Tuple]` in a function signature (emitted `blink_Option_void`); a top-level tuple `let`; a data-enum-returning function materialized into a temp; `match` over a tuple-returning **call** (as opposed to a tuple literal); and a `let x = match …` in value position collapsing to `void`. A `match` over a side-effecting **scalar** scrutinee now binds it to a temp so it's evaluated once, not once per arm.
