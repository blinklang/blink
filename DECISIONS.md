...
| Iterator representation (qzdz2e) | `Iterator[T]` is a **sealed, opaque, built-in carrier**, not a user trait. It is a restartable *recipe* (`Unit -> Option[(T, Iterator[T])]`), not a mutable cursor; layout unspecified, prints as `Iterator[T]`. Supersedes the "Iterator trait / adapter placement" rows below | 6-0 |
| ~~Iterator trait~~ | ~~Single `next(self) -> Option[T]` method~~ — **superseded by qzdz2e** (`next()` is unimplementable under by-value `self`; carrier is sealed) | 3-2 (historical) |
| IntoIterator separation | Separate `IntoIterator` trait; collections produce iterators, aren't iterators. **Sealed in v1** (only built-in collections implement it); user `IntoIterator` deferred to v2 (qzdz2e) | 5-0; sealing 4-2 (qzdz2e R2) |
| Eager collections vs lazy iterators (qzdz2e) | Collection adapters are **eager**, answer a collection (`list.map(f) -> List[U]`); `Iterator` adapters are **lazy**, answer an iterator. `.into_iter()` crosses in, `.collect()` crosses back. No implicit conversion either way | 5-1 (Min: uniform-lazy) |
| ~~Iterator adapter placement~~ | ~~Default methods on `Iterator` trait~~ — **superseded by qzdz2e**: adapters are a sealed built-in method surface (`IteratorOps`), not user-overridable defaults | 5-0 (historical) |
| Custom iteration (qzdz2e) | `iter.from_fn`/`iter.unfold` free functions are the **sole v1 producers**; the `Iterator` trait is sealed and unimplementable. `from_fn` carriers are one-shot; `.into_iter()`-derived carriers restart | 4-2 R2 (DevOps/Min: open `IntoIterator` in v1) |
| `zip`/`enumerate` result (qzdz2e) | Return tuples — `Iterator[(T, U)]` and `Iterator[(Int, T)]` — never nested lists | 6-0 |
| Effectful iteration | Deferred to v2. v1 `Iterator` is pure (effectful adapter callback = error); the effect row is **reserved in the internal carrier representation only**, invisible on every v1 surface signature. Effectful iteration is a conservative v2 extension (qzdz2e) | 3-2 defer (historical); reserve-invisibly 6-0 (qzdz2e) |
...
| FFI pointer type model | `Ptr[T]` non-null default, `Ptr[T]?` for nullable via `Option`. `Void` opaque type for `Ptr[Void]`. No const/mut distinction. **Nullability point superseded by "`Ptr[T]` operations table reconciliation" below (non-null by convention; `Ptr[T]?` reserved).** | 4-1 (Sys/Web/PLT/DevOps for B; AI for A) |
| FFI pointer operations | Minimal + deref/write/null: `alloc_ptr`, `as_cstr`, `addr`, `deref() -> Option[T]`, `write(T)`, `is_null()`, `null_ptr[T]()`, `to_str()`. **`deref`/`addr`/`null_ptr` return types superseded by "`Ptr[T]` operations table reconciliation" below.** | 5-0 |
...
| `Ptr[T]` operations table reconciliation | One canonical ops table in §9.1.1: `deref() -> T` bare (not `Option[T]`), `addr() -> Int` (numeric; `&ptr`/`Ptr[Ptr[T]]` meaning deleted), `null_ptr[T]() -> Ptr[T]` bare (not `Ptr[T]?`), `offset(i) -> Ptr[T]`; `to_str() -> Option[Str]` kept (Option earned). `Ptr[Void]` deref/write rejected via **E0825**. `Ptr[T]` "non-null by convention, not by proof"; `Ptr[T]?` reserved. Supersedes the deref/addr/null_ptr/nullability points of the two rows above | 6-0 (Q1/Q3/Q4/R-void/R-verb; Q2 substance 6-0, name `addr` 4-2 soft-consensus vs `as_int`) |
...
| Mocking helpers beyond IO captures (`mock_clock`, `mock_env`) | Ship `mock_clock(start: Instant) -> MockClock` and `mock_env(initial: Map[Str, Str]) -> MockEnv` in central `std.testing` as **controller structs** (not free-fn factories). `MockClock` exposes `.handler() -> Handler[Time]`, `.advance(d: Duration)`, `.elapsed() -> List[Duration]`. `MockEnv` exposes `.handler() -> Handler[Env]` (with `Env.exit(code)` panicking with the captured code rather than terminating the test runner), `.set(name, value)`, `.writes() -> List[(Str, Str)]`. Per-instance struct state isolates correctly under nested `with` blocks (module-level `mut` would break leftmost-wins semantics at §4:1020). Rule: free-fn factory for stateless sinks (capture_log family), controller for stateful mocks. `record_calls[E]` rejected outright — constraint #9 (no effect-kinded generics in v1) makes it inexpressible | 5-1 Q1 R2 scope; A3, min dissent A2 (was 4-1-1 R1), 5-1 Q2 placement; B1, sys dissent B2, 5-1 Q3 R2 mock_clock shape; C2, min dissent C1 (was 4-2 R1), 4-2 Q4 R2 mock_env shape; D2, devops/min dissent D1 (was 3-3 R1 tie), 6-0 Q5 reject record_calls |
| Deterministic randomness in tests (`--seed`, `mock_rand`) | `blink test --seed <u64>` flag (hex `0x…` or decimal) with `rerun: blink test --seed 0x… --filter '<name>'` line in every failure mode (human + NDJSON `seed`/`reproduce` fields). Per-property sub-seed = `splitmix64(suite_seed XOR siphash24(property_name))`; duplicate property names within a module are a runner error. In-test seeded RNG is a `MockRand` controller struct (`fn handler() -> Handler[Rand]`, `fn draws() -> Int` debug-only, `fn reseed(seed: U64)`) constructed by `mock_rand(seed: U64) -> MockRand` (j21b6c family rule); `seeded_rng` rejected. PRNG implementation-defined (xoshiro256\*\*), spec keeps the door open for 2jersy. Persisted counterexamples, `--rerun-failed`, and `unseeded_rand_in_test` lint deferred | 5-1 Q1 `--seed` (min dissent: CI footgun, prefer rerun-line + future regression file), 6-0 Q1-sub derivation, 6-0 Q2 controller, 6-0 Q3 naming, 6-0 Q4 resolve-now |
| `Buf[U8]` FFI bridge runtime representation | One generic `Buf[T]^σ` (no `Buf[U8]` sibling type), σ-tied to enclosing `ffi.scope`, malloc'd (no GC, no finalizer), bridge alphabet `{U8}` as a closed language-version constant. Sealed user surface: no `.len()`, no `.as_ptr()`, no `.read`/`.write`, no public constructors. V3 carve-out: `Buf` nameable inside `@ffi.fn`/`@ffi.struct` and via inferred let-bindings only; `E0822` everywhere else in user Blink-typed code. Free-return bridge primitives `libc.copy_to_buf`, `copy_from_buf`, `copy_from_buf_n` (truncating); length internal to the runtime struct. `W0816` source-deterministic warning fires at `@ffi` decls using non-bridge-alphabet `T`, per-decl suppressible via `@allow(W0816)`, help text never references future versions. Curated `libc.recv_bytes`/`read_bytes`/`getentropy_bytes` ship in v1 as the primary user-facing API. `bytes-bridge` audit category with `bridge-call` and `buf-mention` subcategories and `--no-unaudited-bridges` CI flag | 6-0 SYNTHESIS-R5b (no dissent; no Phase D) |
| v1 `libc.*_bytes` wrapper minimum set (A2 ship gate) | Ratified v1 set = exactly five: `read_bytes`, `recv_bytes`, `write_bytes`, `send_bytes`, `getentropy_bytes` (file/socket × read/write quadrants + standalone CSPRNG); blocks v1 release. Built on `Bytes.with_ptr` (NOT the byte-bridge — read pins `Bytes.zeroed(n)`+`slice(0,got)`, write pins caller's `Bytes` read-only); `copy_to_buf`/`copy_from_buf_n` reserved exclusively for the sealed-`Buf` path; `@ffi` call stays inline in the `with_ptr` closure. Directional return-typing: Bytes-out → `Result[Bytes, Errno]` (`.len()`=count, short read=success); Bytes-in → `Result[Int, Errno]` (count written). Error = thin transparent zero-cost `Errno(Int)` newtype (rich `IoError` is a separate post-v1 additive task; no `NetError` rework). No `flags` arg in v1 (`flags=0` internal). Normative naming law: `<posix>_bytes`, POSIX arg order with `(buf,len)`→trailing `max:Int`/`data:Bytes`, direction-bound return, `! IO`, `with_ptr` mechanism; vectored/`msghdr`/non-`Bytes`-buffer syscalls out of scope. `read_fully` → `std.io` (pure Blink, ships v1, doc cross-ref from `read_bytes`), not a libc member. New `byte-pin` audit subcategory under `bytes-bridge` covers all `with_ptr` pins. Growth gate: added only on demonstrated high-frequency Bytes-in/out usage where `alloc_n` is prohibitive, never speculative. Deferred: recvfrom/sendto (post-v1 UDP gate + `SockAddr`), pread/pwrite (demonstrated-demand), readv/writev (needs `iovec` bridge) | 5-1 Q1 set=C/5 (min dissent → 3, soft consensus via naming-law + demand-gate); 6-0 Q2 `with_ptr` (after Phase D — §07.534 spec fact flipped a 4-2 BRIDGE result); 6-0 Q3 `Errno` newtype; 6-0 Q4 `read_fully`→std.io; 6-0 Q5 naming law normative; Q6 demand-gate recorded |
| `?` operator in test bodies | Test body using `?` is implicitly elaborated to `Result[Void, TestError]` — no annotation form admitted. `TestError { message: Str, error_type: Str, origin: SourceLocation }` is sealed; each `?` site lowers `Err(e)` via `Display.display(e)`. `Display[E]` is a hard requirement at each `?` site (E0512). `blink check` and `blink test` apply one rule (resolves knhwcf). Failure NDJSON: `status: "failed"`, closed `cause` enum (`"assertion" \| "propagated_error"`). HOF closures (`for_each`, `prop_check`) obey §3c.2 Rules 2/3 against their own return type; elaboration does not propagate inward. Annotation form `test "name" -> Result[Void, E] { }` explicitly rejected, with documented reopen path | 6-0 Q1/Q2/Q3/Q4/Q6; 5-1 Q5 reject annotation (AI/ML: defer) |
| `prop_check` property-closure `?` (generic-E follow-up) | The property closure given as the **direct syntactic argument** of the `prop_check` intrinsic inherits the §2.20 `Result[Void, TestError]` elaboration on its own account — `?` works with no annotation and no `Ok(())`, each `?` site renders via `Display[E]` and stamps `TestError.error_type` with the static name of `E` at that site (distinct sites may carry distinct `error_type`). This does **not** reopen `ka3pw4` Q6: elaboration attaches only to the closed compiler-intrinsic surface (runner is sole caller), so `for_each` and any `let`-bound closure passed to `prop_check` still obey §3c.2 Rules 2/3. Runner ABI stays monomorphic `fn(in) -> Result[Void, TestError]` (render-at-`?`-site; generic `E` never reaches the runner — rejects β's generic-E carrier and γ2 vtable). A returned `Err` is a property failure: the shrinker minimizes the input identically to an assertion failure, and the failure block prints the rendered error beneath `shrunk input:`. `?` on `Option` operands is subsumed (None → `TestError{message:"None", error_type:"Option"}`); no separate `Option[Void]`-returning closure form. NDJSON reuses the closed `cause` enum (`"assertion" \| "propagated_error"`); a prop `?`-failure record additionally carries `seed`/`shrunk_input`/`reproduce`. `.unwrap()`-do-nothing (γ) rejected: routes a falsified property to `status:"panicked"`, breaking the reproduce UX | 6-0 Q1 α (β rejected, γ withdrawn by Min); Q2 Option subsumed (consensus); 6-0 Q3 reuse closed `cause` enum |
| Polymorphic builtin-generic trait impls | Compile via **monomorphization** — each instantiation emits a separate function; no runtime type-shape intrinsics (`size_of[T]`, `is_pointer_kind[T]`, `TypeRepr[T]`) exist, even as `@compiler_internal`; stdlib layout queries route through `@ffi`. Spec contains a normative **parametricity rule**: user impl bodies must be parametric in T (no dispatching on T's identity, no inspecting T's runtime shape); trait-bounded method calls are explicitly carved out as allowed. Diagnostic for parametricity violations must lead with concrete impossibility, cite the spec rule via `note:`, and propose a trait-bound alternative via `help:` | 6-0 Q1 direction; 5-1 Q2 drop intrinsics (sys dissent → `@compiler_internal`); 4-2 Q3 spec sentence (web/devops dissent → diagnostic-only; soft consensus, dissenters endorse underlying need) |
| Multi-cleanup test registrar | Reject `testing.scope()` registrar for now. Conditional from parent ticket (8y2s5x) has not fired: zero callers of existing `testing.cleanup`, no MVCE of real pyramid pain. Shipping a `defer`-shaped registrar risks the same unconscious-pattern-matching hazard that motivated the original defer rejection. Reopening trigger: friction ticket with MVCE showing ≥3 distinct test sites needing 3+ cleanups, AND documented attempt with plain `List[fn() -> Void]` proving insufficient, AND RFC locking the API shape + spec text. When `exit(false)` itself panics on stacked `with...as`, remaining handlers still drain (6-0); secondary panic surfaces as E0824 warning, original unwind preserved | 4-2 Q1 reject (sys/web dissent → ship Scope now, soft consensus on reopening trigger); 6-0 Q2 continue-drain; 6-0 Q3 reopening criteria |
| Trait sealed-default method semantics | Generalize `final` as a method-level modifier on trait default methods. Defaults are **overridable by default**; trait author opts into sealing per-method with `final`. `final` on a body-less method is a parse error (E0732 FinalRequiresBody). Override semantics: **replace-only** — no super-call to reach the default from inside an override. Sealing is **monotonic under trait extension**: a supertrait's `final` cannot be un-sealed in a subtrait. Open override effect rows must satisfy `R_o ⊆ R_d` against the default's row. Stdlib seals `Eq.ne` and `Sized.is_empty` (definitional derived views of required methods). Open→sealed migration: `@deprecate_override` attribute + `W0731 OverrideOfDeprecatedDefault` warning hop, then flip to `final` with `E0731 SealedMethodOverride`. Sub-trait re-sealing (strengthening an open supertrait default in a subtrait) deferred | 5-1 Q1 generalize `final` (min dissent → keep Display-specific, R2); 5-1 Q2 seal Eq.ne and Sized.is_empty (web dissent → open both); 5-1 Q3 replace-only override (web dissent → super-call); 6-0 Q4 monotonic inheritance; 6-0 Q5 effect-row subtype; 5-1 Q6 deprecate-override migration (min dissent → defer migration story) |
| Sub-trait re-sealing (strengthening) | **Reject.** A subtrait may not re-declare an open supertrait default to seal it. `final fn m { body }` (or any redeclaration of `m`) in a subtrait over an open supertrait default `m` is rejected with **`E0733 SubtraitMethodRedeclaration`** at the declaration site. Sealing is monotonic in one direction only — a method's sealed-ness is fixed at the trait that first declares its body; no subtrait may un-seal (E0731, already decided) or strengthen (E0733) it. Rejected on **coherence**: with no MRO, a re-sealed `m` would dispatch differently through the `SuperTrait` view vs the `SubTrait` view for the same receiver. Framed as a **permanent rule**, not a deferral; help text names both legal fixes (seal at the supertrait; or override normally in the impl). Closes the 344dnd Phase-B deferral | 6-0 Q1 reject + E0733; 6-0 Q2 permanent-rule framing |
| Expected-failure tests + parking | Formalize `.tmp/<ticket>/` parking workflow with `blink test --parked <ticket>` flag, file-level `parked_file` NDJSON event, cancel-cascade suppression under `--parked`, and a mechanical lint over `.tmp/` in `task ci`. Ship runtime xfail via `test.failing("name", reason: Str, ticket: Str) { ... }` with **mandatory non-empty** `reason` + `ticket` (separate fields, no regex). Encode under the existing closed status enum (`passed/failed/panicked/skipped`) and closed `cause` enum: add two optional record fields `expected_fail: Bool` and `xfail_reason: Str`. Expected red → `status:"passed"`, `expected_fail:true`; unexpected pass → `status:"failed"`, `cause:"assertion"`, `assertion:"expected failure, got pass"` (strict-by-default, no soft mode). Closed-ticket lint via `br` integration runs in `task ci` (downgrades to warning if `br` unreachable). Four-case decision rule: doesn't build → park; builds-runs-red (deliberately not yet implemented) → `test.failing`; builds-runs-red (bug) → ordinary `test`; should not run yet → `skip()` | 5-1 Q1 parking workflow (PLT dissent → docs-only convention); **3-3 → user BDFL** Q2 ship-now (Blink users actively requesting); 4-2 Q3 boolean `expected_fail` encoding (rejected: new top-level status; new `cause` value) |
| Hash seed contract + iteration order + `--deterministic` | `Map`/`Set` hash seed **randomized per process by default** (ratifies shipped runtime); spec says "randomized," not "unspecified." `hash()` is a pre-seed value satisfying seed-free Eq/Hash coherence (H1); the seed perturbs only bucketing, never observable/stored/serialized, write-once pre-`main`, not an effect and no seed-reading API (H2). Iteration order ∉ value-identity; any fn iterating a `Map`/`Set` is conservatively excluded from §4.16.3 memoization. `--deterministic` flag (pins 0; wired into `build`/`run`/`test`) + `BLINK_MAP_SEED` env (decimal, runtime, crosses process boundaries); precedence `--deterministic` > env > entropy; contract in §3.6, mechanisms in §8.10 (not normative §3). `--seed` (test RNG, §8.10.4) is independent — does not pin map order (cross-ref only). Float keys rejected permanently as `E1400 MapKeyNotHashable`. Ship `W1401 MapOrderAssumption` lint (default-on warning, intraprocedural/direct-flow, machine-applicable `.sorted()` fix) | 6-0 Q1 randomized; 6-0 Q2 keep flag+env / §8.10 placement; 6-0 Q3 test randomized + `--deterministic` opt-in; **3-2-1 → user broke tie** Q3-coupling `--seed` RNG-only (cross-ref, no coupling); 6-0 Q4 purity exclusion; 4-2 Q5 one normative Float sentence (AI/ML+min dissent, accepted as floor); 6-0 Q6 H1/H2 + seed lifecycle; 5-1 Q7 ship lint (min dissent → defer until friction) |
| Bridge alphabet is a language-version constant | The byte-bridge alphabet `BridgeAlpha(λ)` (`{U8}` in v1) is fixed by the **language version** — the compiler/spec revision reported by `blink --version`, NOT the per-package `edition` (§8.16.1) and NOT package semver. No new section or versioning axis: harden three guarantees as prose in §9.1.3.2 + name the language-version/edition dual inline in §8.16.1. **G1** alphabet is a function of language version alone (source-deterministic `W0816`); **G2** expansion = panel deliberation + DECISIONS.md + version bump, monotonically non-shrinking (conservative extension); **G3** no third-party/stdlib/edition/link-time extension, closed **by construction** (no extension syntax exists). Alphabet membership is **non-gating** — `Buf[T]` type-checks for all `T`; the alphabet only decides `W0816` firing. **No `E08xx`** reserved — a code would presuppose an extension construct and risk LLM hallucination; "no mechanism" is the stronger guarantee. Machine-queryable: `blink --version --json` `bridge_alphabet`/`bridge_alphabet_extensible:false` (normative, CI-assertable) + derived `llms.txt language:` stanza, all projections of one compiler constant asserted equal by a build-time test. W0816 text self-contained ("in this version of Blink," never "wait for v2"), deep-links `blink doc bytes-bridge`. *iff-rationale:* source-determinism holds **iff** the alphabet is keyed on language version alone — edition-keying breaks cross-package confluence (subject-reduction across a package edge); stdlib-keying re-creates the rejected W08xx free-variable leak (`-Werror` non-reproducible) | 6-0 Q1 Option B named-inline (R2 after 3-3 R1 A/B split; sys+web+plt migrated A→B in debate); 6-0 Q2 no E08xx (plt conceded); 6-0 Q3 both surfaces from one constant |
| Built-in method-surface traits are prelude + sealed | `StrOps`/`BytesOps`/`StringBuildOps`/`Sized`/`Contains`/`ListOps`/`MapOps`/`SetOps`/`Joinable` are compiler-known ⇒ in the prelude: names visible (usable in generic bounds) AND dispatch is codegen-intrinsic on built-in receivers (no import). They are **sealed** — user `impl`/redefinition is a compile error (seal currently unenforced; latent-bug filed). §3.2.2 table fixed (StrOps/BytesOps added; StringBuildOps→Yes; Sized gains Bytes; `Contains` element membership is `Set`-only today, List/Map planned). §3.2.2:249 "StringBuilder requires explicit import" struck. §10.6 prose broadened + cross-ref; importing a prelude name is a no-op. `[T: StrOps]` legal but degenerate (single-impl) | 6-0 Q1 model-a; 6-0 Q2 Contains row matches reality + bug; 6-0 Q3 no new category; 6-0 Q4 import no-op; 6-0 Q5 sealed hard-error + latent-bug; 4-2 Q6 bless bounds (web/min: stay silent → soft-consensus "legal + single-impl note") |
| `@derive(Debug)` container field rendering | `List[T]`/`Option[T]`/`Map[K,V]` implement `Debug` **conditionally** (iff their type argument(s) are `Debug`); the `<?>` placeholder is removed. Format (debug-form elements/keys/values; `Str` quoted+escaped): List `"[a, b, c]"` (empty `"[]"`); Option `"Some(x)"` / `"None"` (None bare, no parens); Map `"{k: v}"` (empty `"{}"`), `, ` between elements/entries, `: ` between map key and value, no inner padding. A non-Debug element/key/value type is rejected with **`E0520`** naming the offending inner type (`Map` requires both `K` and `V`); **no placeholder** for this case (would relocate the banned silent fallback one level down). **One container level for v1**: a nested container (`List[List[T]]`, `Map[K, List[V]]`, …) is a **hard `E0520` at depth+1, never a placeholder** (message names the v1 limit + tracking ticket). Soundness lives in the inductive typecheck rule (recurses to prove every nested element is `Debug`); the fully-recursive per-monomorphization `debug()` emitter is the deferred end-state (follow-up ticket; mirror the `blink_promote_{mangle}` descriptor emitter). §3.6 trait table + new *Container Debug Rendering* subsection | 6-0 Q1 implement Debug / drop `<?>`; 6-0 Q2 format (independent convergence); 6-0 Q3 reject non-Debug element via extended E0520; 3-3 R1 → **5-1 R2** Q4 one-level (sys dissent → full; web/aiml flipped after Phase D); AI-First 0/5 fail |
| `Char` debug-form | `Char.debug()` = the character in **single quotes**, escaping exactly the ratified `Char` literal set (`\n \r \t \\ \b \f \0 \'`); every other scalar (printable ASCII + all non-ASCII) emitted as its literal UTF-8 char between the quotes. `'a'`→`'a'`, `'\n'`→`'\n'`, `'\''`→`'\''`, `'😀'`→`'😀'`. **Changes the current `97`** (bare code point — an accidental fall-through of `Char` into the `is_hash_scalar_ct → int_to_str` numeric bucket; never a designed contract). Rationale is injectivity/faithfulness: `'a'`→`97` collapses `Char 'a'` onto `Int 97` (and would make `Map[Char,Int]{'a':1}` byte-identical to `Map[Int,Int]{97:1}`), violating the 5-0 faithfulness ban and silently doing the explicit-only `Char→Int` coercion; single-quoting also keeps `Char` distinct from a 1-char `Str` (`'a'`≠`"a"`) and consistent with "`Char` is not numeric" (§3c). Invariant: `Char.debug()` emits only escapes the lexer accepts, so `parse(c.debug()) == c` for every `c` in the ratified set (round-trip). **Control-char `'\u{N}'` output deferred-and-linked to task 19v5gb** (adds `\u{...}` as input syntax): until then a non-printable scalar with no named escape (e.g. `U+0007`) is emitted as its raw byte(s) — faithful, not always legible. Flows unchanged into `Char` struct fields / `List[Char]` / `Option[Char]` / `Map[Char,V]` keys via one shared helper (they agree by construction). §3.6 Debug table + *Debug vs Display* scalar debug-forms paragraph | 6-0 Q1 single-quoted (Option A over keep-`97` / double-quote-as-Str / bare-char); 6-0 Q2 raw-glyph for exotic controls, `\u{N}` deferred to 19v5gb (plt conceded A-hex→A-raw); AI-First 5/5 pass |
| FFI import namespace resolution | `ffi` stays a no-import intrinsic namespace (like `io`/`net`/`time`); `import blink.ffi`/`import blink.core` resolve as **recognized, inert, optional no-ops** — never required, never `ModuleNotFound`. Selective names (`Ptr`/`Void`/`alloc_ptr`/`null_ptr`) are satisfiable no-ops. **No E/W import gate**: the unsafe surface is gated only by `PtrOutsideFFI` (E0811 — `Ptr[T]` inside `@ffi`/`@trusted`) and `#275` (`@ffi` requires a `[native-dependencies]` manifest entry). Non-breaking; no lib/std or test migration. (br `q0brkj`, 2026-07-07) | 6-0 |
| Enums nominally distinct from `Int` | Delete the enum↔Int assignability carve-out in `types_compatible`: an enum is **not** assignable to/from `Int` at let-binding, argument, or return (falls to `ka == kb` → 0, mirroring the sized-int rule). **Comparison stays legal** (`==`/`<`… short-circuit to `Bool` before `types_compatible`, comparing the shared tag). Explicit conversion only: `Enum.to_int()` (total) / `Enum.from_int(n) -> Option[Enum]` (fallible); **no `as` cast operator** added. Migration is a **hard flip** (no deprecation phase) — the 16 affected sites are all compiler-internal and their fixes (`new_node(kind: NodeKind)`, `Token.kind: TokenKind`, `peek_kind` return, `_last_kind` sentinel) advance the planned `kind:Int → NodeKind` migration. **Match-scrutinee** enforcement (`match someInt { Variant => }` ill-typed) is part of the rule but its enforcement is **staged** behind the `kind:Int → NodeKind` migration (br `qsb7ca` ← `0khtje`). `Errno(Int)` stays an enum, distinct for free. Out of scope (br `x231pe`): struct-literal field values are unchecked. §3.4 *Enums Are Nominally Distinct from `Int`* | 6-0 Q1 delete carve-out (Option A); 5-1 Q2 stage match-scrutinee (PLT dissent → NOW, feasibility-conceded); 6-0 Q3 Errno stays enum; AI-First 5/5 pass |
| Under-determined types are a hard error (no silent default, no surface `unknown`) | Inference is **two-state**: every type variable resolves to a concrete type, or the unresolved ones are **reported** — never defaulted. When Hindley-Milner finishes a binding with a type variable still unbound (not fixed by annotation or by later use), that binding is **`error[CannotInferType]` (E0301)**, applied uniformly to empty `[]`, bare `None`, empty `Map()`/`Set()`, and under-constrained generic construction (`GKV { m: Map() }`) — one rule, one diagnostic. **No surface `unknown`/`any`/`?` type**: `TK_UNKNOWN` stays a transient internal inference state, never nameable/holdable/inferred into a program (TS `unknown` is opted-into, never *produced* by inference; a top type in a monomorphizing backend forces boxing/RTTI). **Strict, no unobserved-local carve-out**: `let x = []; x.len()` errors (an element-agnostic use does not constrain the element); making legality depend on whether the value is later observed needs a whole-function escape analysis that breaks locality of reasoning and is refactor-fragile. **Report-at-binding, dual-span**: primary caret + machine-applicable annotation fix at the `let` (`: List[T]` / `: Map[K,V]` / `: T?`, ctor shape known), secondary "blame" span at the empty ctor (rejects OCaml weak-var deferred-to-boundary reporting — fires far from source, no single fix). **ICE backstop `UnsolvedTypeVarAtCodegen` (I0001)**: an unsolved `TK_TYPEVAR`/`TK_UNKNOWN` reaching `tc_tid_to_c_tag`/`type_name_from_ct` must fail closed, never fall through to `"Void"` (must key on the *kind*, not the resulting `"Void"` tag, so a genuine `GKV[Void,Void]` is unaffected). Pre-metavar accident (non-uniform: `Map()`→wrong `kops_str` vtable, `None`→arbitrary `blink_Option_int`, `GKV{m:Map()}`→`blink_GKV_Void_Void`+runtime `ensure_kops` patch). Migration: mechanical annotation (`blink fix`-able); sweep compiler's own sources before the flip (self-host). **Status: implemented** — E0301 + I0001 land the decision; the `ensure_kops` runtime patch is retired (ctor-time pinning is now the sole mechanism). **AMENDED by *Explicit type application* (below, V1-c, 6-0)**: report-at-binding is generalized to *report where the repair attaches* — for a type parameter the signature does not supply, that is the call's type-argument position, and "the single repair is a type annotation" is narrowed to bindings. §3.4 *Under-Determined Types*; catalog E0301 + I0001 | 6-0 Q1 error/never-default; 6-0 Q2 reject surface `unknown`; 6-0 Q3 strict no carve-out (plt+sys withdrew Phase-A carve-outs in debate); 6-0 Q4 report-at-binding dual-span; 6-0 Q5 ICE tripwire; AI-First 5/5 pass |
| Explicit type application, and an undeterminable type parameter is a lint not an error | **Bracketed call-site type application is part of Blink** and §3.4 now defines it — a *third* supply mechanism beside inference at a construction site and an annotation on the binding (ratifies ~50 attestations across 8 spec files, incl. a diagnostic's own `help:` text and normative FFI signatures `alloc_ptr[T]() -> Ptr[T]`; §2.6's 5-0 bracket vote already foreclosed an index operator to pay for the slot). **No erasure**: every binder a declaration introduces stays in its monomorphization key — never dropped, defaulted, or collapsed onto another instantiation's C symbol (blocks the ODR collapse where `parse_or_zero[Int]()` and `parse_or_zero[Duration]()` share `() -> Int`). **Brackets are mandatory** at every call of a type parameter the signature does not supply; *supplied by the signature* = occurs in a parameter type or the return type, decidable from the signature alone with no call site and no body consulted. **E0301 is reported where its repair attaches** (amends 8vcj2c) — the type-argument position for an unsupplied binder, including a call in statement position, which previously fell through to I0001. **Two repairs in a normative (not presentational) order**: delete the binder when the lint says it is removable, else supply the type argument — deletion named first, so an LSP one-click cannot launder the defect into working code. **One discipline for all type-argument lists**: all-or-none, exact arity, bounds checked, permitted on a struct-literal head (`Registry[User] { entries: [] }`). **A redundant type-argument list carries no diagnostic of any class** (a warning would be non-monotonic — a later annotation elsewhere would retroactively make unchanged code noisy). **Phantom type parameters are legal in user code** (`type Template[C]`'s 4-1 is not reserved to compiler-known types). **No new declaration-site error**: a `fn`/`type` binder that occurs nowhere gets **`W0604 UnusedTypeParamBinder`**, because `∀a. Int` has the witness `f[Int]()` and a type system may not reject a declaration some well-typed use inhabits. The **normative gate is the occurrence clause** — *`T` occurs nowhere in the declaration or its body* — with fix-safety ("W0604 fires exactly when deleting the binder is a safe edit") adopted as **normative rationale** beside it; **a bound is an occurrence**, so `fn assert_serializable[T: Serialize]() {}` is a compile-time assertion and never warns. **An impl binder is still rejected outright (E0909)** and a `where`-clause bound does not rescue it — reconciled by: reject a declaration only when *no* use site could ever repair it; where one can, the diagnostic goes to the use site and the declaration gets a lint. E0909's shipped explain text ("no call site could ever determine what it stands for") is **deleted** as the false generalization this ticket was filed on. New §3.1 ***Diagnostic Discipline* (normative)**: never emit a diagnostic whose prescribed repair does not exist; no rule enforced only at codegen (I0001 catches violations of that rule, it is never one); co-firing diagnostics must be confluent and the converging repair must be named first. §3.1 *Diagnostic Discipline*, §3.4 *Explicit Type Application*, §3.6 impl-binder rule; catalog E0301 (spec-ref corrected §3.3→§3.4) + new W0604 | 6-0 V1 ratify+no-erasure; 6-0 V1-b normative fix order; 6-0 V1-c report-where-repair-attaches; 6-0 V2 bound-is-an-occurrence; **6-0 R2 V3** occurrence clause as gate / fix-safety as rationale (after 4-1-1 R1 — all four fix-safety voters moved, each off a reason it had authored); 5-1 V4 brackets on a struct-literal head (aiml: one spelling per site); 6-0 V5 no diagnostic on a redundant list; 6-0 V6 bound does not rescue an impl binder; 6-0 V7(i) delete the E0909 sentence / 5-1 V7(ii) keep `ImplBinderUnused` (aiml: rename to `ImplBinderUndeterminable`) / 6-0 V7(iii) E0909 stays; 5-1 V8 "no diagnostic whose repair does not exist" (sys: route to the catalog preamble); 6-0 V9 T1 + confluence; 6-0 V10 code allocation; R5-a declaration-site error does not land (4 NO, 2 YES conditional on an antecedent that lost 6-0) |
| `fs` operations return `Result[T, FsError]`, not a bare `Str` | The four v1 filesystem operations are `fs.read(path) -> Result[Str, FsError] ! FS.Read`, `fs.write(path, content) -> Result[Void, FsError] ! FS.Write`, `fs.list_dir(path) -> Result[List[Str], FsError] ! FS.Read`, `fs.delete(path) -> Result[Void, FsError] ! FS.Delete`. **Error type is a per-domain rich `FsError { op: FsOp, path: Str, code: Errno }`** with `FsOp { Read Write ListDir Delete }` — matching the `net`→`NetError` / `db`→`DbError` convention, *not* a bare `Errno` (the stdlib already runs per-domain errors, so bare-Errno would be the special case; and a path is permanently unrecoverable once `?` propagates past the call frame — Rust's `fs_err` crate is the ecosystem receipt). `code: Errno` retained as the transparent zero-cost OS code (A1: `libc.*` / `io.*` keep bare `Errno`; this does not add a general `IoError`). `FsError` **impl `Display`** (Go-style `op path: message`, a same-change blocker) and ships a `not_found()` predicate; `FsError` + `FsOp` are **prelude-promoted**. **Names follow the spec, not the impl**: `fs.list_dir` (Q3a — listing is `FS.Read`, there is no `FS.List`), `fs.delete` + `FS.Delete` (Q3b — matched verb/effect pair; the impl's `fs.remove` / `FS.Remove` rejected). **No file-handle API in v1** (Q2): exactly the four path ops, no `fs.open` / `fs.create` / `fs.read(handle)`; streaming/incremental access is post-v1 as *methods on a file handle* (`file.read_line()`), never an `fs.read` overload (Blink has no function overloading). `?` is exact (no implicit `.into()`); cross-error adaptation uses `.map_err`. **Riders**: W0603 `UnusedResult` lint same-change (`let _ =` opt-out); `map_err` is a v1-blocking dependency (spec'd, currently unimplemented); user-facing named errno constants deferred, but the internal errno→message+NAME table ships as `Display`'s data source. **Staged implementation** (user directive): fix silent-miscompile `cz10bb` (`Errno` struct field constructed flat into a boxed field) + `fsr5ew` (whole-value `Err(e)` binder on `Result[T, Errno]`) → the C2 "box struct error arms by pointer" codegen rule (guarded by a `Result[Bytes, Errno]` transparent-emit pin-test) → `map_err` → the `FsError` fs-intrinsics. §4.4.5 *Filesystem Operations* | Q1 6-0 (`FsError`, after 3-3 R1 → 6-0 R2); Q2 6-0 (no handle API); Q3a 6-0 (`fs.list_dir`); Q3b 5-1 (web dissent: `fs.remove`); riders Ra/Rb/Rd 6-0 same-change, Re/Rf tracked; AI-First 5/5 pass |
...
| Iterator Protocol (incl. qzdz2e: sealed carrier + eager collections) | [decisions/iterator-protocol.md](decisions/iterator-protocol.md) |
...
| `Ptr[T]` Operations Table | [decisions/ptr-operations-table.md](decisions/ptr-operations-table.md) |
| `Buf[U8]` FFI Bridge Runtime Representation | [decisions/buf-u8-runtime-representation.md](decisions/buf-u8-runtime-representation.md) |
| Bridge Alphabet Is a Language-Version Constant | [decisions/bridge-alphabet-language-version.md](decisions/bridge-alphabet-language-version.md) |
| v1 `libc.*_bytes` Wrapper Minimum Coverage (A2 Ship Gate) | [decisions/libc-bytes-wrapper-coverage.md](decisions/libc-bytes-wrapper-coverage.md) |
...
| Mocking Helpers Beyond IO Captures | [decisions/mocking-helpers-beyond-io.md](decisions/mocking-helpers-beyond-io.md) |
| Test Seed Determinism (`--seed`, `mock_rand`) | [decisions/test-seed-determinism.md](decisions/test-seed-determinism.md) |
| `?` Operator in Test Bodies | [decisions/test-block-question-mark.md](decisions/test-block-question-mark.md) |
| `prop_check` Property-Closure `?` | [decisions/prop-check-question-mark.md](decisions/prop-check-question-mark.md) |
| Polymorphic Builtin-Generic Trait Impls | [decisions/polymorphic-builtin-generic-impls.md](decisions/polymorphic-builtin-generic-impls.md) |
...
| Sub-trait Re-sealing (Strengthening) | [decisions/subtrait-re-sealing.md](decisions/subtrait-re-sealing.md) |
...
| Hash Seed Contract, Iteration Order & `--deterministic` | [decisions/hash-seed-iteration-order.md](decisions/hash-seed-iteration-order.md) |
| Built-in Method-Surface Traits: Prelude Status & Sealing | [decisions/prelude-method-surface-traits.md](decisions/prelude-method-surface-traits.md) |
| `@derive(Debug)` Container Field Rendering | [decisions/debug-container-rendering.md](decisions/debug-container-rendering.md) |
| `Char` Debug-Form | [decisions/char-debug-form.md](decisions/char-debug-form.md) |
| Enums Nominally Distinct from `Int` | [decisions/enum-int-nominal-distinctness.md](decisions/enum-int-nominal-distinctness.md) |
| FFI Import Namespace Resolution | [decisions/ffi-import-namespace-resolution.md](decisions/ffi-import-namespace-resolution.md) |
| Under-Determined Types Are a Hard Error | [decisions/under-determined-types.md](decisions/under-determined-types.md) |
| Explicit Type Application & the Undeterminable Type Parameter | [decisions/explicit-type-application.md](decisions/explicit-type-application.md) |
| Filesystem `fs.read` Return Type (`Result[T, FsError]`) | [decisions/fs-read-return-type.md](decisions/fs-read-return-type.md) |
