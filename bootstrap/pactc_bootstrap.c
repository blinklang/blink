...
int64_t pact_is_builtin_type(const char* name);
void pact_add_type_dep(const char* type_name, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed);
void pact_walk_body_deps(int64_t node, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed);
void pact_walk_stmts_deps(int64_t stmts_sl, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed);
void pact_collect_fn_deps(int64_t fn_node, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed);
void pact_check_pub_sig(int64_t fn_node, const char* fn_name, pact_map* mod_types_pub);
...
int64_t pact_is_builtin_type(const char* name) {
    if ((((((pact_str_eq(name, "") || pact_str_eq(name, "Int")) || pact_str_eq(name, "Str")) || pact_str_eq(name, "Float")) || pact_str_eq(name, "Bool")) || pact_str_eq(name, "Void"))) {
        return 1;
    }
    if (((((pact_str_eq(name, "List") || pact_str_eq(name, "Map")) || pact_str_eq(name, "Option")) || pact_str_eq(name, "Result")) || pact_str_eq(name, "Iterator"))) {
        return 1;
    }
    return 0;
}
...
void pact_add_type_dep(const char* type_name, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed) {
    if ((pact_is_builtin_type(type_name) == 1)) {
        return;
    }
    if (pact_map_has(needed, type_name)) {
        return;
    }
    if (pact_map_has(mod_types, type_name)) {
        pact_map_set(needed, type_name, (void*)(intptr_t)(int64_t)(intptr_t)pact_map_get(mod_types, type_name));
        const int64_t tnode = (int64_t)(intptr_t)pact_map_get(mod_types, type_name);
        const int64_t flds = (int64_t)(intptr_t)pact_list_get(np_fields, tnode);
        if ((flds != (-1))) {
            int64_t fi = 0;
            while ((fi < pact_sublist_length(flds))) {
                const int64_t fld = pact_sublist_get(flds, fi);
                const int64_t fkind = (int64_t)(intptr_t)pact_list_get(np_kind, fld);
                if ((fkind == pact_NodeKind_TypeField)) {
                    const int64_t ta = (int64_t)(intptr_t)pact_list_get(np_value, fld);
                    if ((ta != (-1))) {
                        pact_add_type_dep((const char*)pact_list_get(np_name, ta), mod_fns, mod_types, mod_lets, needed);
                    }
                } else if ((fkind == pact_NodeKind_TypeVariant)) {
                    const int64_t vflds = (int64_t)(intptr_t)pact_list_get(np_fields, fld);
                    if ((vflds != (-1))) {
                        int64_t vi = 0;
                        while ((vi < pact_sublist_length(vflds))) {
                            const int64_t vf = pact_sublist_get(vflds, vi);
                            const int64_t vta = (int64_t)(intptr_t)pact_list_get(np_value, vf);
                            if ((vta != (-1))) {
                                pact_add_type_dep((const char*)pact_list_get(np_name, vta), mod_fns, mod_types, mod_lets, needed);
                            }
                            vi = (vi + 1);
                        }
                    }
...
void pact_walk_body_deps(int64_t node, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed) {
    ...
    if ((kind == pact_NodeKind_Call)) {
        const int64_t callee = (int64_t)(intptr_t)pact_list_get(np_left, node);
        if (((callee != (-1)) && ((int64_t)(intptr_t)pact_list_get(np_kind, callee) == pact_NodeKind_Ident))) {
            const char* cname = (const char*)pact_list_get(np_name, callee);
            if ((pact_map_has(mod_fns, cname) && (pact_map_has(needed, cname) == 0))) {
                pact_map_set(needed, cname, (void*)(intptr_t)(int64_t)(intptr_t)pact_map_get(mod_fns, cname));
                const int64_t dep_fn = (int64_t)(intptr_t)pact_map_get(mod_fns, cname);
                pact_collect_fn_deps(dep_fn, mod_fns, mod_types, mod_lets, needed);
            }
        }
        pact_walk_body_deps(callee, mod_fns, mod_types, mod_lets, needed);
        const int64_t args_sl = (int64_t)(intptr_t)pact_list_get(np_args, node);
        if ((args_sl != (-1))) {
            int64_t ai = 0;
            while ((ai < pact_sublist_length(args_sl))) {
                pact_walk_body_deps(pact_sublist_get(args_sl, ai), mod_fns, mod_types, mod_lets, needed);
                ai = (ai + 1);
            }
        }
        return;
    ...
    if ((kind == pact_NodeKind_StructLit)) {
        const char* stype = (const char*)pact_list_get(np_type_name, node);
        pact_add_type_dep(stype, mod_fns, mod_types, mod_lets, needed);
        const int64_t sfields = (int64_t)(intptr_t)pact_list_get(np_fields, node);
        if ((sfields != (-1))) {
            int64_t si = 0;
            while ((si < pact_sublist_length(sfields))) {
                const int64_t sf = pact_sublist_get(sfields, si);
                pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, sf), mod_fns, mod_types, mod_lets, needed);
                si = (si + 1);
            }
        }
        return;
    ...
    if ((kind == pact_NodeKind_Ident)) {
        const char* ref_name = (const char*)pact_list_get(np_name, node);
        if ((pact_map_has(mod_lets, ref_name) && (pact_map_has(needed, ref_name) == 0))) {
            pact_map_set(needed, ref_name, (void*)(intptr_t)(int64_t)(intptr_t)pact_map_get(mod_lets, ref_name));
            const int64_t let_node = (int64_t)(intptr_t)pact_map_get(mod_lets, ref_name);
            pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, let_node), mod_fns, mod_types, mod_lets, needed);
        }
        return;
    }
    if ((kind == pact_NodeKind_BinOp)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_left, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_right, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_UnaryOp)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_left, node), mod_fns, mod_types, mod_lets, needed);
    ...
    if ((kind == pact_NodeKind_IfExpr)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_condition, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_then_body, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_else_body, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_Block)) {
        pact_walk_stmts_deps((int64_t)(intptr_t)pact_list_get(np_stmts, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_LetBinding)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_ExprStmt)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, node), mod_fns, mod_types, mod_lets, needed);
    ...
    if (((kind == pact_NodeKind_Assignment) || (kind == pact_NodeKind_CompoundAssign))) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_target, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_ForIn)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_iterable, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_body, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_WhileLoop)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_condition, node), mod_fns, mod_types, mod_lets, needed);
    ...
    if ((kind == pact_NodeKind_MatchExpr)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_scrutinee, node), mod_fns, mod_types, mod_lets, needed);
        const int64_t arms_sl = (int64_t)(intptr_t)pact_list_get(np_arms, node);
        if ((arms_sl != (-1))) {
            int64_t ai = 0;
            while ((ai < pact_sublist_length(arms_sl))) {
                const int64_t arm = pact_sublist_get(arms_sl, ai);
                pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_guard, arm), mod_fns, mod_types, mod_lets, needed);
                pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_body, arm), mod_fns, mod_types, mod_lets, needed);
                ai = (ai + 1);
            }
        }
        return;
    ...
    if ((kind == pact_NodeKind_IndexExpr)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_obj, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_index, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if (((kind == pact_NodeKind_TupleLit) || (kind == pact_NodeKind_ListLit))) {
        const int64_t elems_sl = (int64_t)(intptr_t)pact_list_get(np_elements, node);
        if ((elems_sl != (-1))) {
            int64_t ei = 0;
            while ((ei < pact_sublist_length(elems_sl))) {
                pact_walk_body_deps(pact_sublist_get(elems_sl, ei), mod_fns, mod_types, mod_lets, needed);
                ei = (ei + 1);
            }
        }
        return;
    ...
    if ((kind == pact_NodeKind_InterpString)) {
        ...
        if ((parts_sl != (-1))) {
            int64_t pi = 0;
            while ((pi < pact_sublist_length(parts_sl))) {
                pact_walk_body_deps(pact_sublist_get(parts_sl, pi), mod_fns, mod_types, mod_lets, needed);
                pi = (pi + 1);
            }
        }
        return;
    ...
    if ((kind == pact_NodeKind_RangeLit)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_start, node), mod_fns, mod_types, mod_lets, needed);
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_end, node), mod_fns, mod_types, mod_lets, needed);
        return;
    }
    if ((kind == pact_NodeKind_WithBlock)) {
        pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_body, node), mod_fns, mod_types, mod_lets, needed);
...
void pact_walk_stmts_deps(int64_t stmts_sl, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed) {
    ...
    int64_t i = 0;
    while ((i < pact_sublist_length(stmts_sl))) {
        pact_walk_body_deps(pact_sublist_get(stmts_sl, i), mod_fns, mod_types, mod_lets, needed);
        i = (i + 1);
    }
}

void pact_collect_fn_deps(int64_t fn_node, pact_map* mod_fns, pact_map* mod_types, pact_map* mod_lets, pact_map* needed) {
    const char* ret_type = (const char*)pact_list_get(np_return_type, fn_node);
    pact_add_type_dep(ret_type, mod_fns, mod_types, mod_lets, needed);
    const int64_t params_sl = (int64_t)(intptr_t)pact_list_get(np_params, fn_node);
    if ((params_sl != (-1))) {
        int64_t pi = 0;
        while ((pi < pact_sublist_length(params_sl))) {
            const int64_t p = pact_sublist_get(params_sl, pi);
            pact_add_type_dep((const char*)pact_list_get(np_type_name, p), mod_fns, mod_types, mod_lets, needed);
            pi = (pi + 1);
        }
    }
    pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_body, fn_node), mod_fns, mod_types, mod_lets, needed);
}

void pact_check_pub_sig(int64_t fn_node, const char* fn_name, pact_map* mod_types_pub) {
    const char* ret_type = (const char*)pact_list_get(np_return_type, fn_node);
    if ((((pact_is_builtin_type(ret_type) == 0) && pact_map_has(mod_types_pub, ret_type)) && ((int64_t)(intptr_t)pact_map_get(mod_types_pub, ret_type) == 0))) {
        char _si_0[4096];
        snprintf(_si_0, 4096, "pub fn '%s' returns non-pub type '%s'", fn_name, ret_type);
        char _si_1[4096];
        snprintf(_si_1, 4096, "make '%s' pub or change the return type", ret_type);
        pact_diag_error_at("VisibilityError", "E1300", strdup(_si_0), fn_node, strdup(_si_1));
    }
    const int64_t params_sl = (int64_t)(intptr_t)pact_list_get(np_params, fn_node);
    if ((params_sl != (-1))) {
        int64_t pi = 0;
        while ((pi < pact_sublist_length(params_sl))) {
            const int64_t p = pact_sublist_get(params_sl, pi);
            const char* ptype = (const char*)pact_list_get(np_type_name, p);
            if ((((pact_is_builtin_type(ptype) == 0) && pact_map_has(mod_types_pub, ptype)) && ((int64_t)(intptr_t)pact_map_get(mod_types_pub, ptype) == 0))) {
                char _si_2[4096];
                snprintf(_si_2, 4096, "pub fn '%s' has parameter of non-pub type '%s'", fn_name, ptype);
                char _si_3[4096];
                snprintf(_si_3, 4096, "make '%s' pub or change the parameter type", ptype);
                pact_diag_error_at("VisibilityError", "E1300", strdup(_si_2), fn_node, strdup(_si_3));
            }
            pi = (pi + 1);
        }
    }
...
int64_t pact_merge_programs(int64_t main_prog, pact_list* imported, pact_list* import_nodes_list) {
    ...
    while ((pi < pact_list_len(imported))) {
        ...
        const int is_selective = ((int64_t)(intptr_t)pact_list_get(np_args, imp_node) != (-1));
        if (is_selective) {
            pact_map* mod_fns = pact_map_new();
            pact_map* mod_types = pact_map_new();
            pact_map* mod_lets = pact_map_new();
            pact_map* mod_types_pub = pact_map_new();
            ...
            while ((fi < pact_sublist_length(fns_sl))) {
                ...
                pact_map_set(mod_fns, (const char*)pact_list_get(np_name, f), (void*)(intptr_t)f);
                fi = (fi + 1);
            }
            ...
            while ((ti < pact_sublist_length(types_sl))) {
                ...
                pact_map_set(mod_types, (const char*)pact_list_get(np_name, t), (void*)(intptr_t)t);
                pact_map_set(mod_types_pub, (const char*)pact_list_get(np_name, t), (void*)(intptr_t)(int64_t)(intptr_t)pact_list_get(np_is_pub, t));
                ti = (ti + 1);
            }
            ...
            while ((li < pact_sublist_length(lets_sl))) {
                ...
                pact_map_set(mod_lets, (const char*)pact_list_get(np_name, l), (void*)(intptr_t)l);
                li = (li + 1);
            }
            pact_map* needed = pact_map_new();
            fi = 0;
            while ((fi < pact_sublist_length(fns_sl))) {
                const int64_t f = pact_sublist_get(fns_sl, fi);
                if ((pact_should_import_item(f, imp_node) == 1)) {
                    const char* fname = (const char*)pact_list_get(np_name, f);
                    pact_map_set(needed, fname, (void*)(intptr_t)f);
                    if (((int64_t)(intptr_t)pact_list_get(np_is_pub, f) != 0)) {
                        pact_check_pub_sig(f, fname, mod_types_pub);
                    }
                    pact_collect_fn_deps(f, mod_fns, mod_types, mod_lets, needed);
                }
                fi = (fi + 1);
            }
            ti = 0;
            while ((ti < pact_sublist_length(types_sl))) {
                ...
                if ((pact_should_import_item(t, imp_node) == 1)) {
                    const char* tname = (const char*)pact_list_get(np_name, t);
                    pact_map_set(needed, tname, (void*)(intptr_t)t);
                    pact_add_type_dep(tname, mod_fns, mod_types, mod_lets, needed);
                }
                ti = (ti + 1);
            }
            li = 0;
            while ((li < pact_sublist_length(lets_sl))) {
                ...
                if ((pact_should_import_item(l, imp_node) == 1)) {
                    pact_map_set(needed, (const char*)pact_list_get(np_name, l), (void*)(intptr_t)l);
                    pact_walk_body_deps((int64_t)(intptr_t)pact_list_get(np_value, l), mod_fns, mod_types, mod_lets, needed);
                }
                li = (li + 1);
            }
            ...
            while ((tri < pact_sublist_length(traits_sl))) {
                ...
                if ((pact_should_import_item(tr, imp_node) == 1)) {
                    pact_map_set(needed, (const char*)pact_list_get(np_name, tr), (void*)(intptr_t)tr);
                }
                tri = (tri + 1);
            }
            fi = 0;
            while ((fi < pact_sublist_length(fns_sl))) {
                const int64_t f = pact_sublist_get(fns_sl, fi);
                if (pact_map_has(needed, (const char*)pact_list_get(np_name, f))) {
            ...
            while ((ti < pact_sublist_length(types_sl))) {
                const int64_t t = pact_sublist_get(types_sl, ti);
                if (pact_map_has(needed, (const char*)pact_list_get(np_name, t))) {
            ...
            while ((li < pact_sublist_length(lets_sl))) {
                const int64_t l = pact_sublist_get(lets_sl, li);
                if (pact_map_has(needed, (const char*)pact_list_get(np_name, l))) {
            ...
            while ((tri < pact_sublist_length(traits_sl))) {
                const int64_t tr = pact_sublist_get(traits_sl, tri);
                if (pact_map_has(needed, (const char*)pact_list_get(np_name, tr))) {
